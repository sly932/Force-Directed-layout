        ...
        while True:
            ...
            for x, node_x in enumerate(self.nodes):
                for y, node_y in enumerate(self.nodes):
                    if x != y:
                        if args.withmass:
                            dx, dy,_ = node_x - node_y
                            mx, my = node_x[2], node_y[2]
                            distance = math.sqrt(dx**2 +dy**2)
                            if distance == 0:
                                continue
                            force = args.k_r * mx * my / distance**2
                        else:
                            dx, dy = node_x - node_y
                            distance = math.sqrt(dx**2 +dy**2)
                            if distance == 0:
                                continue
                            force = args.k_r / distance**2
                        force_x, force_y = force * dx / distance, force * dy / distance
                        repulsion[x][0] += force_x
                        repulsion[x][1] += force_y
                        repulsion[y][0] -= force_x
                        repulsion[y][1] -= force_y
            # 计算弹簧作用力
            spring = np.zeros_like(self.nodes)
            for edge in self.edges:
                if edge[1]-edge[0] < 0:
                    continue
                edge = edge - 1
            ...
            for i,node in enumerate(self.nodes):
                ...
                dis = dx**2 + dy**2
                if dis > cur_max_len:
                    rate = math.sqrt(cur_max_len / dis)
                    dx, dy = dx*rate, dy*rate 
                delta_step += abs(dx)
                delta_step += abs(dy) 
                self.nodes[i][0],self.nodes[i][1] = x+dx, y+dy
            if not args.notdraw: 
                delta_sum.append(delta_step)
        return self.nodes
...
def generateMassNodes(node_num,minval,maxval,minmass,maxmass):
    #generate nodes
    #("generating nodes with mass...",end='')
    nodes = []
    for i in range(node_num):
        inter = maxval - minval
        x, y = random.random()*inter+minval,random.random()*inter+minval
...
if __name__ == "__main__":
    ...
    algorithm.execute()
    #algorithm.savedata(0,'latest')
    #algorithm.printNodes(args,0)
