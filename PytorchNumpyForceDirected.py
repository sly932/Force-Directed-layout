...
class ForceDirectedAlgorithm:
    ...
    def execute(self):
        ...
        args.max_move = torch.tensor(args.max_move,dtype=torch.float16).to(device)
        self.nodes = torch.from_numpy(self.nodes).to(device)
        self.edges = torch.from_numpy(self.edges).to(device)
        ...
        with torch.no_grad():
            while True:
                ...
                # 计算斥力
                repulsion = torch.zeros_like(self.nodes,device=device,dtype=torch.float16)
                dn = self.nodes[:,None,:2] - self.nodes[None,:,:2]
                dist = torch.linalg.norm(dn,dim=2).to(device)
                dist[dist==0] = 1e-6
                if args.withmass:
                    m = self.nodes[:,None,2] * self.nodes[None,:,2]
                    repulsion = args.k_r * dn * m[:,:,None] / dist[:,:,None]**3
                else:
                    repulsion = args.k_r * dn / dist[:,:,None]**3
                repulsion = torch.sum(repulsion,dim=1)
                # 计算弹簧作用力
                nodes = self.nodes.cpu().numpy()
                spring = np.zeros_like(nodes[:,:2])
                ...
                for edge in edges:
                    ...
                    edge = edge - 1
                    if args.withmass:
                        dx, dy, _ = nodes[edge[1]] - nodes[edge[0]]
                    else :
                        dx, dy = nodes[edge[1]] - nodes[edge[0]]
                ...
                # 更新节点坐标
                delta = repulsion + torch.from_numpy(spring).to(device)
                cur_max_len = (args.max_move / (step + 0.1)).clone().detach().to(torch.float16)
                dist = torch.sum(delta**2,dim=1)
                idx = dist > cur_max_len
                rate = torch.ones(len(delta),device=device,dtype=torch.float16)
                rate[idx] = torch.sqrt(cur_max_len / dist[idx]).to(torch.float16)
                delta[idx] = delta[idx] * rate[idx,None]
                self.nodes[:,:2] += delta
                if not args.notsave:
                    delta_sum.append(torch.sum(delta).cpu())
        self.printNodes(args,1) 
        return self.nodes.cpu().numpy()
